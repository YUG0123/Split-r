...
export const getTotalSpent = query({
  handler: async (ctx) => {
    ...
    // FIXED: Corrected query index format from "date,startOfYear" to separate arguments
...
export const getMonthlySpending = query({
  handler: async (ctx) => {
    ...
    // FIXED: Changed undefined 'userId' to 'user._id'
    ...
    userExpenses.forEach((expense) => {
      ...
      // FIXED: Changed 'split,userId' to 'split.userId'
      const userSplit = expense.splits.find(
        (split) => split.userId === user._id,
      );
    ...
    // FIXED: Corrected mapping destructuring argument names from [monthlyTotals, total] to [month, total]
    const result = Object.entries(monthlyTotals).map(([month, total]) => ({
      month: parseInt(month),
      total,
    }));

    // FIXED: Fixed broken arrow syntax from '= >' to '=>'
